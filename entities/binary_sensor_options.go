...
type BinarySensorOptions struct {
	...
	deviceClass            string // "Sets the [class of the device](/integrations/binary_sensor/#device-class), changing the device state and icon that is displayed on the frontend. The `device_class` can be `null`."
	...
	name                   string // "The name of the binary sensor. Can be set to `null` if only the device name is relevant."
	...
	platform               string // "Must be `binary_sensor`. Only allowed and required in [MQTT auto discovery device messages](/integrations/mqtt/#device-discovery-payload)."
...
func (o *BinarySensorOptions) EntityPicture(picture string) *BinarySensorOptions {
...
func (o *BinarySensorOptions) Platform(platform string) *BinarySensorOptions {
	o.platform = platform
	return o
}
