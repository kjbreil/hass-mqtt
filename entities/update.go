...
func (d *Update) UpdateState() {
	if d.AvailabilityTopic != nil {
		...
		stateStore.Update.Mutex.Unlock()
	}
	if d.JsonAttributesTopic != nil {
		state := d.JsonAttributesFunc()
		stateStore.Update.Mutex.Lock()
		if state != stateStore.Update.JsonAttributes[d.GetUniqueId()] || (d.MQTT.ForceUpdate != nil && *d.MQTT.ForceUpdate) {
			token := (*d.MQTT.Client).Publish(*d.JsonAttributesTopic, byte(*d.Qos), *d.Retain, state)
			stateStore.Update.JsonAttributes[d.GetUniqueId()] = state
			token.WaitTimeout(common.WaitTimeout)
		}
		stateStore.Update.Mutex.Unlock()
	}
